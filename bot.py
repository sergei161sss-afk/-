...
# ─── ГЕОЛОКАЦИЯ ────────────────────────────────────────────────────
BRANCHES_GEO = {
    "Ирий":  (47.2333, 39.7489),
    "Правь": (47.2367, 39.6942),
}
GEO_RADIUS_OK   = 150
GEO_RADIUS_WARN = 500
...
async def cmd_reset(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Сброс профиля — регистрация заново."""
    tid = update.effective_user.id
    users = load_users()
    if str(tid) in users:
        del users[str(tid)]
...
async def on_branch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ГЕО ОТКЛЮЧЕНО ДЛЯ ТЕСТИРОВАНИЯ — включить после проверки
    await q.edit_message_text(
...
async def on_location(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not target:
        await update.message.reply_text("Филиал не найден. Напишите /start.",
                                        reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    dist = haversine(loc.latitude, loc.longitude, target[0], target[1])
    if dist <= GEO_RADIUS_OK:
        status_msg = f"✅ Вы на месте ({int(dist)} м)"
    elif dist <= GEO_RADIUS_WARN:
        status_msg = f"⚠️ Немного далеко ({int(dist)} м). Продолжаем."
    else:
        if ADMIN_CHAT_ID:
            await ctx.bot.send_message(
                ADMIN_CHAT_ID,
                f"🔴 {s['name']} ({ROLE_SHORT.get(s['role'], s['role'])}) пытается открыть смену "
                f"в {branch}, но находится в {int(dist)} м!"
            )
        await update.message.reply_text(
            f"❌ Вы слишком далеко от {branch} ({int(dist)} м). Смена не открыта.",
            reply_markup=ReplyKeyboardRemove(),
        )
        return ConversationHandler.END
...
async def on_main(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    btns = _item_buttons(items, prefix, s) + [("↩️ Назад", "back")]
...
async def on_qty(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    amt = price * new_qty
    await q.edit_message_text(
        f"*{name}*\nКоличество: {_qty_str(new_qty)}  →  {int(amt):,} ₽\n\nВыберите количество:",
        parse_mode="Markdown",
        reply_markup=kb([
            ("+1",   "qty_plus1"),
            ("+0.5", "qty_plus05"),
...
async def cmd_setup_rv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for name, sid in RV_FILES.items():
        try:
            ...
            ws = sh.worksheet("Журнал") if "Журнал" in existing else \
                 sh.add_worksheet(title="Журнал", rows=2000, cols=13, index=0)
    ...
    await update.message.reply_text(
        "Результат:\n" + "\n".join(results) +
        "\n\n📌 Открой каждый РВ → лист «Журнал» → «Разрешить доступ»"
    )
        ...
        states={
            ...
            PROFILE_MENU: [
                CallbackQueryHandler(
                    on_profile_menu,
                    pattern="^(go_profile|change_role|setrole_.+|change_name"
                            "|newname_.+|confirmname_.+|start_shift)$",
                ),
            ],
            CHOOSE_BRANCH: [
            ...
            MAIN_MENU: [
                CallbackQueryHandler(
                    on_main,
                    pattern="^(show|close|add_rate|add_proc|profile"
                            "|change_role|setrole_main_.+|back_main)$",
                ),
            ],
            ADD_ITEM: [
                CallbackQueryHandler(on_add, pattern="^(r_\\d+|p_\\d+|back)$"),
            ],
            ADD_QTY: [
